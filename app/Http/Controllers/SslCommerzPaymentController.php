...
class SslCommerzPaymentController extends Controller
{
    ...
    public function success(Request $request)
    {
        ...
        $ssl_data = Sslorder::where('transaction_id',$tran_id)->get()->first();
        echo $ssl_data->transaction_id;

        $order_id = '#'.Str::upper(Str::random(3)).'-'.rand(000000,999999);
        Order::create([
            'order_id' => $order_id,
            'customer_id' => $ssl_data->customer_id,
            'subtotal' => $ssl_data->subtotal,
            'discount' => $ssl_data->discount,
            'charge' => $ssl_data->charge,
            'total' => $request->input('amount'),
            'payment_method' => $ssl_data->payment_method
        ]);
        ...
        BillingDetail::create([
            ...
            'customer_id' => $ssl_data->customer_id,
            'name' => $ssl_data->name,
            'email' => $ssl_data->email,
            'company' => $ssl_data->company,
            'phone' => $ssl_data->phone,
            'address' => $ssl_data->address,
            'zip_code' => $ssl_data->zip_code,
            'country_id' => $ssl_data->country_id,
            'city_id' => $ssl_data->city_id,
            'notes' => $ssl_data->notes,

        ]);

        $carts = Cart::where('customer_id',$ssl_data->customer_id)->get();
        ...
        //Invoice
        Mail::to($ssl_data->email)->send(new InvoiceMail($order_id));


        Cart::where('customer_id',$ssl_data->customer_id)->delete();
